...
import * as raydium from "./abi/raydium-cpmm"
...
const dataSource = new DataSourceBuilder()
  ...
  .addInstruction({
    ...
    where: {
      programId: [raydium.programId], // where executed by Whirlpool program
      d8: [raydium.instructions.swapBaseInput.d8], // have first 8 bytes of .data equal to swap descriptor
...
run(dataSource, database, async (ctx) => {
  console.log("Processing blocks from slot:", ctx.blocks[0].header.slot)
  ...
  for (let block of blocks) {
    console.log("Block:", block.header.slot)
    for (let ins of block.instructions) {
      // https://read.cryptodatabytes.com/p/starter-guide-to-solana-data-analysis
      if (ins.programId === raydium.programId && ins.d8 === raydium.instructions.swapBaseInput.d8) {
